...
import com.agaroth.world.content.skill.impl.runecrafting.DesoSpan;
...
import com.agaroth.world.content.skill.impl.summoning.BossPets;
...
import com.agaroth.world.content.skill.impl.thieving.Pickpocket;
		...
		player.setWalkToTask(new WalkToTask(player, npc.getPosition(), npc.getSize(), new FinalizedMovementTask() {
			...
			public void execute() {
				...
				case 501:
					...
					player.setDialogueActionId(175);
					break;
				case 2862:
				...
				case 211:
					...
					break;
				case 462:
					DialogueManager.start(player, 148);
					break;
				case 586:
					DialogueManager.start(player, 140);
					player.setDialogueActionId(141);
					break;
				case 284: 
				...
				case 8705:
					...
					break;
				case 947: 
			          if (Misc.getMinutesPlayed(player) < 15 && player.getGameMode() != GameMode.HARDCORE_IRONMAN && player.getGameMode() != GameMode.IRONMAN) {
			            player.getPacketSender().sendMessage("You must have played for at least 15 minutes in order to use Grand Exchange.");
			            return;
			          }
			          if (player.getRights() == PlayerRights.ADMINISTRATOR) {
			            player.getPacketSender().sendMessage("Administrators cannot use Grand Exchange.");
			            return;
			          }
				...
				case 6970:
					...
					break;
				case 4657: //Sir Prysin
					player.setDialogueActionId(134);
			        DialogueManager.start(player, 134);
					//player.getPacketSender().sendMessage("@red@Check out weekly donator store discount by using command ::discount");
					break;
				case 318:
				case 316:
				case 313:
		...
		player.setWalkToTask(new WalkToTask(player, npc.getPosition(), npc.getSize(), new FinalizedMovementTask() {
			...
			public void execute() {
				...
				case 3777:
					...
					break;
				case 501:
					TeleportHandler.teleportPlayer(player, new Position(3108, 3352), TeleportType.NORMAL);
				...
				case 2115: 
			          if (player.getAmountDonated() <= 6) {
			        	  player.getPacketSender().sendMessage("You must be donator to pickpocket gem trader.");
			          	return;
			          }
			          double c = Math.random() * 100.0;
			          int reward = c >= 7.0 ? 1617 : c >= 25.0 ? 1619 : c >= 40.0 ? 1621 : c >= 70.0 ? 1623 : c >= 90.0 ? 1629 : c >= 95.0 ? 1627 : c >= 97.0 ? 1625 : 1631;
			          Pickpocket.stealFromNPC(player, 93, 12800, reward, "You stole a gem.");
			          break;
			    case 4657: 
	...
	public void handleThirdClick(Player player, Packet packet) {
		...
		player.setWalkToTask(new WalkToTask(player, npc.getPosition(), npc.getSize(), new FinalizedMovementTask() {
			...
			public void execute() {
				...
				case 961:
					if(player.getAmountDonated() <= 6) {
						player.getPacketSender().sendMessage("This feature is currently only available for donators only.");
						return;
					}
	...
	public void handleFourthClick(Player player, Packet packet) {
		...
		player.setWalkToTask(new WalkToTask(player, npc.getPosition(), npc.getSize(), new FinalizedMovementTask() {
			...
			public void execute() {
				switch(npc.getId()) {
				case 3777:
					if(player.getRights() == PlayerRights.DONATOR || player.getRights() == PlayerRights.SUPER_DONATOR || player.getRights() == PlayerRights.EXTREME_DONATOR || player.getRights() == PlayerRights.LEGENDARY_DONATOR || player.getRights() == PlayerRights.UBER_DONATOR || player.getRights() == PlayerRights.DEVELOPER || player.getRights() == PlayerRights.OWNER) {
						TeleportHandler.teleportPlayer(player, new Position(3363, 9649), player.getSpellbook().getTeleportType());
					} else
						player.getPacketSender().sendMessage("You must be a donator to access this zone.");
						return;
				case 462:
					EnergyHandler.rest(player);
					break;
				case 4657:
					if(player.getAmountDonated() <= 6) {
						player.getPacketSender().sendMessage("You need to be a donator to teleport to this zone.").sendMessage("To become a donator, contact Juice or do ::donate.");
						return;
					}
					TeleportHandler.teleportPlayer(player, new Position(3363, 9649), player.getSpellbook().getTeleportType());
					break;
