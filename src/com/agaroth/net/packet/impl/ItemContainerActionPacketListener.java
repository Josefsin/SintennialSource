...
import com.agaroth.model.input.impl.EnterAmountToBank;
import com.agaroth.model.input.impl.EnterAmountToBuyFromShop;
import com.agaroth.model.input.impl.EnterAmountToPriceCheck;
import com.agaroth.model.input.impl.EnterAmountToRemoveFromBank;
import com.agaroth.model.input.impl.EnterAmountToRemoveFromBob;
import com.agaroth.model.input.impl.EnterAmountToRemoveFromPriceCheck;
import com.agaroth.model.input.impl.EnterAmountToRemoveFromStake;
import com.agaroth.model.input.impl.EnterAmountToRemoveFromTrade;
import com.agaroth.model.input.impl.EnterAmountToSellToShop;
import com.agaroth.model.input.impl.EnterAmountToStake;
import com.agaroth.model.input.impl.EnterAmountToStore;
import com.agaroth.model.input.impl.EnterAmountToTrade;
...
import com.agaroth.world.content.grandexchange.GrandExchangeOffer;
	...
	private static void thirdAction(Player player, Packet packet) {
		...
		case Equipment.INVENTORY_INTERFACE_ID:
			...
			case 11124:
				...
				break;
			case 20314: 
		        if (System.currentTimeMillis() - player.SwitchBookTimer < 60000) {
		          player.getPacketSender().sendMessage("@red@Your staff needs 60 second cooldown to be operated again.");
		          return;
		        }
			...
			case 15492:
			case 13263:
				player.getPacketSender().sendMessage("Your current task is to kill "+player.getSlayer().getAmountToSlay()+" "+Misc.formatText(player.getSlayer().getSlayerTask().toString().toLowerCase().replaceAll("_", " "))+"s.");
				break;
			case 11283: 
		        int charges = player.getDfsCharges();
		        if (System.currentTimeMillis() - player.DFSTimer < 30000) {
		          player.getPacketSender().sendMessage("@red@Your shield needs 30 second cooldown to be operated again.");
		          return;
		        }
		        if ((charges >= 1) || (player.getRights() == PlayerRights.DEVELOPER) || (player.getRights() == PlayerRights.OWNER)) {
		          if (player.getCombatBuilder().isAttacking()) {
		            player.DFSTimer = System.currentTimeMillis();
		          ...
		          player.getPacketSender().sendMessage("Your shield doesn't have enough power yet. It has " + player.getDfsCharges() + "/30 dragon-fire charges.");
		        }
		        break;
		      }
		      break;
