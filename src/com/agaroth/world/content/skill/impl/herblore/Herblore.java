...
public class Herblore {
	...
	static enum SpecialPotionM
	  {
	    EXTREME_ATTACK(new Item[] { new Item(145), new Item(261) }, new Item(15308), 88, 4430),  EXTREME_STRENGTH(new Item[] { new Item(157), new Item(267) }, new Item(15312), 88, 4753),  EXTREME_DEFENCE(new Item[] { new Item(163), new Item(2481) }, new Item(15316), 90, 5002),  EXTREME_MAGIC(new Item[] { new Item(3042), new Item(9594) }, new Item(15320), 91, 5408),  EXTREME_RANGED(new Item[] { new Item(169), new Item(12539, 5) }, new Item(15324), 92, 5924),  OVERLOAD(new Item[] { new Item(15309), new Item(15313), new Item(15317), new Item(15321), new Item(15325) }, new Item(15332), 96, 13103);
	    ...
	    private SpecialPotionM(Item[] ingridients, Item product, int lvlReq, int exp)
	    {
	      this.ingridients = ingridients;
	      this.product = product;
	      this.lvlReq = lvlReq;
	    ...
	    public static SpecialPotionM forItems(int item1, int item2)
	    {
	      SpecialPotionM[] arrayOfSpecialPotionM;
	      int j = (arrayOfSpecialPotionM = values()).length;
	      for (int i = 0; i < j; i++)
	      {
	        SpecialPotionM potData = arrayOfSpecialPotionM[i];
	        int found = 0;
	        Item[] arrayOfItem;
	        int m = (arrayOfItem = potData.getIngridients()).length;
	        for (int k = 0; k < m; k++)
	        {
	          Item it = arrayOfItem[k];
	          if ((it.getId() == item1) || (it.getId() == item2)) {
	            found++;
	          }
	        }
	...
	public static void handleSpecialPotion(Player p, int item1, int item2) {
		...
		if(!p.getInventory().contains(item1) || !p.getInventory().contains(item2)) 
			return;
		  SpecialPotionM specialPotDataM = SpecialPotionM.forItems(item1, item2);
		...
		if (p.getAmountDonated() <= 6) {
		      p.getInventory().add(specialPotData.getProduct());
		    } else {
		      p.getInventory().add(specialPotDataM.getProduct());
		    }
