...
public class CharmingImp {
	...
	private static void turnIntoXp(Player player, int itemId, int amount) {
		switch (itemId) {
		case GOLD_CHARM:
			player.getSkillManager().addExperience(Skill.SUMMONING, 6544 * amount);
			break;
		case GREEN_CHARM:
			player.getSkillManager().addExperience(Skill.SUMMONING, 8444 * amount);
			break;
		case CRIM_CHARM:
			player.getSkillManager().addExperience(Skill.SUMMONING, 10877 * amount);
			break;
		case BLUE_CHARM:
			player.getSkillManager().addExperience(Skill.SUMMONING, 12544 * amount);
			break;
		}
	...
	public static void sendConfig(Player player) {
		for(int i = 0; i < 4; i++) {
			...
			switch(state) {
			case 0:
				player.getPacketSender().sendMessage("<img=11> <col=996633>Your Charming imp is placing all "+ItemDefinition.forId(charm).getName()+"s it finds in your inventory.");
				break;
			case 1:
				player.getPacketSender().sendMessage("<img=11> <col=996633>Your Charming imp is turning all "+ItemDefinition.forId(charm).getName()+"s it finds into Summoning exp.");
				break;
			}
		}
	}
