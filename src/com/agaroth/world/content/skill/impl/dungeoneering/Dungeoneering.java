	...
	public static void handleNpcDeath(Player p, NPC n) {
		if(n.getPosition().getZ() == p.getPosition().getZ()) {
			...
			if(random >= 20 && random <= 45 || boss) {
				...
				if(boss) {
					party.sendMessage("@red@The boss has been slain! Your floor will be abandoned in 5 minutes.");
					party.sendMessage("@red@Please bind items you would like to keep, before time runs out.");
					TaskManager.submit(new Task(5000, p, false) {
						@Override
						public void execute() {
							Dungeoneering.leave(p, false, false);
							stop();
						}
					});
				}
