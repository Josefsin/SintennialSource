	...
	public static void bindItem(Player p, int item) {
		if(Dungeoneering.doingDungeoneering(p)) {
			...
			if(amountBound >= 4) {
				...
				return;
			} else if(amountBound == 3 && p.getSkillManager().getCurrentLevel(Skill.DUNGEONEERING) < 99) {
				p.getPacketSender().sendMessage("You need a Dungeoneering level of at least 99 to have 4 bound items.");
				return;
