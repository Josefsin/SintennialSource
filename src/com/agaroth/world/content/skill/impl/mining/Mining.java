					...
					if (miningLevel >= p.getReq()) {
						...
						player.setCurrentTask(new Task(delay >= 2 ? delay : 1, player, false) {
							...
							public void execute() {
								...
								if(giveGem) {
									boolean onyx = (o == Ores.Runite || o == Ores.CRASHED_STAR) && Misc.getRandom(o == Ores.CRASHED_STAR ? 30000 : 7000) == 1;
									if(onyx || Misc.getRandom(o == Ores.CRASHED_STAR ? 35 : 60) == 15) {
										...
										if(gemId == 6571) {
											...
											World.sendMessage("<img=11><col=009966> "+player.getUsername()+" has just received an Uncut Onyx from mining a "+s+"!");
										}
									}
								}
								if (cycle == reqCycle) {
