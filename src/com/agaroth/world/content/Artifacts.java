...
public class Artifacts {
	...
	public static void sellArtifacts(Player c) {
		...
		if(!artifact) {
			c.getPacketSender().sendMessage("You do not have any Artifacts in your inventory to sell to Sloane.");
			return;
		}
	...
	public static void handleDrops(Player killer, Player death, boolean targetKill) {
		...
		if(Misc.getRandom(100) >= 99) {
			...
			World.sendMessage("<img=11><col=009966> "+killer.getUsername()+" has just received "+Misc.anOrA(itemName)+" "+itemName+" from Bounty Hunter!");
		}
	}
