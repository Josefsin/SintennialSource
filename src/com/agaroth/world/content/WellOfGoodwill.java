...
public class WellOfGoodwill {
	...
	public static void donate(Player player, int amount) {
		...
		if(amount > 10000000) {
			World.sendMessage("<img=11> <col=6666FF>"+player.getUsername()+" has donated "+Misc.insertCommasToNumber(""+amount+"")+" coins to the Well of Goodwill!");
		}
		...
		if (getMissingAmount() <= 0) {
			...
			World.sendMessage("<img=11> <col=6666FF>The Well of Goodwill has been filled!");
			World.sendMessage("<img=11> <col=6666FF>It is now granting everyone 2 hours of 30% bonus experience.");
	...
	public static void updateState() {
		if (STATE == WellState.FULL) {
			if(getMinutesRemaining() <= 0) {
				World.sendMessage("<img=11> <col=6666FF>The Well of Goodwill is no longer granting bonus experience.");
